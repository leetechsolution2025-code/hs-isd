...
async function main() {
  ...
  const coords = JSON.parse(project.landmarkCoordinates || '[]');
  ...
  p_n.x = p_n_1.x + u_x * targetDist;
  p_n.y = p_n_1.y + u_y * targetDist;
  
  console.log('New last point:', p_n);
  
  await prisma.project.update({
     where: { id: project.id },
     data: { landmarkCoordinates: JSON.stringify(coords) }
  });
